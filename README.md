...
stable. `baseCru` is `sizeFactor × riskMultiplier`, the cost at full
ownership; `cru` scales that by `ownershipShare` for the final measurement.
...
diffs, a long tail of large ones, and a median noticeably to the right
